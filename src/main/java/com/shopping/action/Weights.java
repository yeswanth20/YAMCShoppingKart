...
public class Weights {
		...
		@Path("/insert") 
		public Response insertWeights(WeightsTo weightsTo,@Context HttpServletRequest request){
			try 
			{
				weightsTo=ShoppingCartFactory.getWeightsDao().insert(weightsTo,12);
	    ...
	    @Consumes(MediaType.APPLICATION_JSON)
		@Path("/update") 
		public Response updateWeight(WeightsTo weightsTo,@Context HttpServletRequest request){
			try 
			{
				weightsTo=ShoppingCartFactory.getWeightsDao().update(weightsTo.getId(),weightsTo,4);
				return Response.status(201).entity(weightsTo).build();	
			}
			catch (Exception e)
			{
	    ...
	    @Consumes(MediaType.APPLICATION_JSON)
		@Path("/search") 
		public Response searchWeight(WeightsTo weightsTo,@Context HttpServletRequest request){
			try 
			{
				weightsTo=ShoppingCartFactory.getWeightsDao().searchById(weightsTo.getId());
				return Response.status(201).entity(weightsTo).build();	
			}
			catch (Exception e)
			{
